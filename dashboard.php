...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'review') {
    ...
    if (
        ...
        flash('Choose a rating and write at least 10 characters about your experience.');
        redirect('dashboard.php#reviews');
    }

    $appointment = $pdo->prepare("SELECT id FROM appointments WHERE id = ? AND user_id = ? AND status = 'Completed' LIMIT 1");
    $appointment->execute([$appointmentId, $user['id']]);

    if (!$appointment->fetch()) {
        flash('A review can only be added to one of your completed appointments.');
        redirect('dashboard.php#reviews');
    }
    ...
    try {
        ...
        flash('Thank you. Your review was sent to the administrator for approval.');
    } catch (PDOException $exception) {
        flash($exception->getCode() === '23000'
...
$completedAppointments = array_values(array_filter(
    $appointments,
